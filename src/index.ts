...
const Store = new Level(resolve(argv.dir, 'filehost'), { valueEncoding: 'binary' });
...
app.get('/:name', (req: Request,res: Response) => {
    ...
    getPackage(packageName).then((pkg:any) => {
        const versions = Object.keys(pkg.versions);
        versions.forEach(version => {
            pkg.versions[version].dist.tarball = `${argv['url']}/tarballs/${pkg.name}/${version}.tgz`
            pkg.versions[version].dist.info = `${argv['url']}/${pkg.name}/${version}`;
        })
...
app.get('/tarballs/:name/:version.tgz', (req: Request,res: Response) => {
    ...
    getPackage(packageName).then((pkg:any) => {
        Store.get(id, {keyEncoding:'buf', valueEncoding: 'binary' }, (err, buf: any) => {
            if(!err) {
                let hash = createHash('sha1');
                hash.update(buf);
                if(pkg.versions[version].dist.shasum !== hash.digest('hex')) {
                    res.status(500).send({
                ...
                axios.get(tgzLocation)
                    .then(content => {
                        res.setHeader('Content-Type', 'application/octet-stream')
                        res.setHeader('Content-Length', content.data.length);
                        res.send(content);    
                    })
                    .catch(err => {
                        res.status(500).send(err)
        ...
        pkg.versions[version].downloads ? pkg.versions[version].downloads += 1 : pkg.versions[version].downloads = 1;
        localSkim.put(pkg);
    })
